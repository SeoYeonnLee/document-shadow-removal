...
class my_dataset(Dataset):
    def __init__(self, root_dir, crop_size=256, fix_sample_A=999, regular_aug=False):
        ...
        in_files = random.sample(in_files, self.fix_sample_A)
        self.imgs_in_A = [os.path.join(input_dir, k) for k in in_files]
        self.imgs_gt_A = [os.path.join(target_dir, k) for k in in_files]
    ...
    def read_imgs_pair(self, in_path, gt_path, transform, crop_size):
        ...
        img_name_A = os.path.basename(in_img_path_A)
    ...
    def train_transform(self, img, label, patch_size=256):
        ...
        label_pil = label_pil.resize((patch_size, patch_size), Image.Resampling.BICUBIC)
