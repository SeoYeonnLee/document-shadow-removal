...
###########################################################################################################################
def multi_scale_losses(outputs, label, loss):
    N_scale = len(outputs)
    labels = [label]
    # labels_img2 = F.interpolate(label, scale_factor= 0.5, mode='bilinear')
    # labels_img4 = F.interpolate(labels_img2, scale_factor= 0.5, mode='bilinear')
    # labels_img8 = F.interpolate(labels_img4, scale_factor= 0.5, mode='bilinear')
    scale_factor = 1
    for i in range(N_scale-1):
        scale_factor = scale_factor * 0.5
        label_img_sub_scale = F.interpolate(label, scale_factor= scale_factor, mode='bilinear')
        labels.insert(0,label_img_sub_scale)
    #total_loss =

    for j in range(N_scale):
        if j == 0:
            total_loss = loss(labels[j], outputs[j])
        total_loss += loss(labels[j], outputs[j])
...
class Vgg19(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        self.vgg.load_state_dict(torch.load('/root/ShadowChallendge/loss/vgg19-dcbb9e9d.pth'))
        self.vgg.eval()
...
class WeightedCharbonnierLoss(nn.Module):
    """WeightedCharbonnierLoss Loss (L1)"""
    def __init__(self, eps=1e-4, weight = 10.0):
    ...
    def forward(self, x, y):
        ...
        mask = (torch.sum(x, dim = 1,keepdim=True) >  0.8).float()  
        
        diff = x - y
        loss = torch.mean(torch.sqrt((diff * diff) + (self.eps*self.eps)))  + self.weight * torch.mean(torch.sqrt((diff * diff *(1 - mask)) + (self.eps*self.eps)))
        return loss
