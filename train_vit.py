"""
Vision Transformer (ViT) Training for Shadow Removal
"""
...
from utils.UTILS import compute_psnr, compute_ssim, print_args_parameters
...
parser.add_argument('--BATCH_SIZE', type=int, default=16, help="Initial batch size")
...
parser.add_argument('--shadow_matte_path', type=str, default="./checkpoints/UNet/image_256/epoch180.pth", help="Path to the shadow matte model weights")
...
wandb.init(project="MatteViT", name=args.experiment_name, config=vars(args))
SAVE_PATH = os.path.join('./checkpoints/ViT', args.experiment_name)
...
net = MaskedAutoencoderViT(
    patch_size=8, in_chans=4, embed_dim=256, depth=6, num_heads=8,
...
pbar = tqdm(range(global_iter, max_iter), initial=global_iter, total=max_iter, desc="Training")
for _ in pbar:
    try:
